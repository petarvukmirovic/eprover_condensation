                         ...
                         RewriteLevel level)
{
   ...
   while(!done)
   {
      ...
      if(control->heuristic_parms.condensing || control->heuristic_parms.fast_condensing)
      {
         fflush(stderr);
         double base = GetTotalCPUTime();
         if(control->heuristic_parms.condensing)
         {
            condensed = Condense(clause);
         }
         else if(control->heuristic_parms.fast_condensing)
         {
            condensed = CondenseSet(clause);
         }
         ...
         state->condensing_time += GetTotalCPUTime() - base;
         fflush(stderr);
      }
