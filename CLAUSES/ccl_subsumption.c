...
/*-----------------------------------------------------------------------
//
// Function: eqn_list_rec_setsubsume()
//
//   Try to find a subset of sub_cand_list such that
//   subst(subsum_list) = subset. Return true if this is possible,
//   false otherwise.
...
static
bool eqn_list_rec_setsubsume(Eqn_p subsum_list, Eqn_p sub_cand_list,
                             Subst_p subst)
{
   Eqn_p         eqn;
   PStackPointer state;
   ...
   for(eqn = sub_cand_list; eqn; eqn = eqn->next)
   {
      if(EqnQueryProp(eqn, EPIsCondensed))
      {
         continue;
      }
      ...
      if(SubstComputeMatch(subsum_list->lterm, eqn->lterm, subst)&&
         SubstComputeMatch(subsum_list->rterm, eqn->rterm, subst))
      {
         if(eqn_list_rec_setsubsume(subsum_list->next, sub_cand_list, subst))
         {
            return true;
         }
      }
...
static bool clause_subsumes_clause(Clause_p subsumer, Clause_p  sub_candidate)
{
   Subst_p subst;
   bool    res;
   long* pick_list;
...
/*-----------------------------------------------------------------------
//
// Function: ClauseSetSubsumesClauseModuloEq()
//
//   Return true if subsumer subsumes sub_candidate, where both
//   susbumer and sub_candidate are considered as sets. Also considers
//   subsumer modulo removal of trivial literals (that is, the literals
//   s != t where subst(s) = subst(t) syntactically)
//
// Global Variables: -
//
// Side Effects    : -
...
bool ClauseSubsumesClauseModuloSetEq(Clause_p subsumer, Clause_p sub_candidate)
{
   ...
   res = eqn_list_rec_setsubsume(subsumer->literals,
                                 sub_candidate->literals, subst);

   //SubstPrint(stderr, subst, subsumer->literals->bank->sig, DEREF_NEVER);
   //fprintf(stderr, "%d \n", res);
   SubstDelete(subst);
